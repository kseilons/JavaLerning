...
public class JavaBank extends JFrame {
    ...
    private CompanyColor companyColor = new CompanyColor();
    ...
    private void CreateAccountJButtonActionPerformed(ActionEvent event) {
    	...
    	//Get Balance from Text Field and convert to int unless blank then set to 0
    		...
    		Name = "";
    		JOptionPane.showMessageDialog(null,  "некорректное числовое значение");
    	}
    	catch(Exception e) {
	...
	private void TransactionJButtonActionPerformed(ActionEvent event) {
    	...
    	if (noAccounts == 0) {
        	...
        	if ((myAccounts.get(i).getaccountnum() == Accountnum) && (Deposit>0)) {
        		...
        		displayJTextArea.setText(myAccounts.get(i).getaccountname() + " " + myAccounts.get(i).getaccountnum() + 
        				" " + myAccounts.get(i).getBalance() + " " + myAccounts.get(i).getType());
        	}
    ...
    private void DisplayJButtonActionPerformed(ActionEvent event) {
    	...
    	if (noAccounts == 0) {
    		...
    		for (int i=0; i<noAccounts; i++) { 
    			displayJTextArea.append(myAccounts.get(i).getaccountname() + " " + myAccounts.get(i).getaccountnum() + 
    					" " + myAccounts.get(i).getBalance() + " " + myAccounts.get(i).getType() + "\n");
    		}
    	}
        // clear other JTextFields for new data
